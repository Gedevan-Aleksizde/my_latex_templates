...
biblio-style: jecon
mainfont: Hiragino Maru Gothic ProN W4
monofont: SF Mono
CJKmainfont: Hiragino Maru Gothic ProN W4
linkcolor: blue
citecolor: blue
urlcolor: magenta
---
...
```yaml
output: rmdCJK::beamer_presentation_CJK
```
...
* yamlヘッダは見本として書いただけで実際は**細かい設定不要**
* 文献引用をしないなら `biblio*` も不要
* 使えるフォントはマシン依存
  + 参考: [Google Fonts](https://fonts.google.com/?category=Sans+Serif#standard-styles)でフリーフォント入手可能
  + インラインでのフォント変更は**未実装**(`rmarkdown`の修正待ち)

```yaml
output:
  rmdCJK::beamer_presentation_CJK:
bibliography: <.bibファイル>
biblio-style: <.bstファイル>
mainfont: <欧文フォント>
monofont: <等幅フォント>
CJKmainfont: <和文フォント>
```
...
* `XeCJK` パッケージで制御している
  + 本当は `zxjatype`でやりたかった
* `mainfont`は**セリフフォント**を指定する必要
