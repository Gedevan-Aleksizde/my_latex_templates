...
date: "`r Sys.Date()`"
output:
  rmdzxja::beamer_presentation_zxja:
    keep_md: false
    theme: metropolis
    jamainfont: Noto Serif CJK JP
    jasansfont: Noto Sans CJK JP
    jamonofont: Ricty Diminished
    mainfont: Roboto Slab
    mainfontoptions: Ligatures=TeX,Scale=MatchUppercase
    sansfont: Roboto
...
file.copy(file.path(system.file(package = "rmdzxja"), "extdata/.latexmkrc"), to = "./")
# 上記2つは日本語引用文献リストのために必要
knitr::opts_chunk$set(cache = T)
extdata <-  file.path(system.file(package = "rmdzxja"), "extdata") # サンプル画像のディレクトリパス
# extdata <- getwd()
```
...
knitr::include_graphics(file.path(extdata, "img/render.png"))
```
...
* インラインでのフォント変更は**想定してない**
  + やりたい人は[ここ](https://astr.phys.saga-u.ac.jp/~funakubo/research/recipe/xelatex-presen.html)や[ここ](http://zrbabbler.sp.land.to/zxjatype.html)等を参考に

```yaml
mainfont: Roboto Slab
sansfont: Roboto
monofont: Ricty Diminished
mainfont-ja: Noto Serif CJK JP
sansfont-ja: Noto Sans CJK JP
monofont-ja: Ricty Diminished
```
...
knitr::include_graphics(file.path(extdata, "img/math-input.png"))
```
* 一部対応してない記号もある?
  + `\mathbb{}`とか`\hat{}`とか
...
knitr::include_graphics(file.path(extdata, c("img/tiger.eps", "img/tiger.pdf", "img/tiger.png")))
```
...
```
![The Tiger](`r file.path(extdata, "img/tiger.pdf")`){ height=30% }
...
xtable::xtable(
  ...
  print(file = file.path(extdata, "tab.tex"))
```
...
* markdown方式のリンク
  ...
  + [RStudio](https://rstudio.com/)
* 画像にハイパーリンク [![RStudio](`r file.path(extdata, "img/RStudio-Logo-flat.pdf")`){ height=10% }](https://rstudio.com/) を貼ることも可
...
knitr::include_graphics(file.path(extdata, "img/citr.png"))
```
...
* `-options` で `fontspec`/`zxjatype`のオプションを使用可能

```yaml
mainfont-options: Ligature=TeX
mainfont-ja-options: BoldFont=Noto Sanc CJK JP
```
...
* A: **キャッシュ削除すると良くなることもある**
  + (**叩けば直る**レベルの雑アドバイス)
  + `{ファイル名}_cache` というディレクトリが作られている
  + 前回失敗した際のキャッシュが悪さしてることは結構ある
  + または `cache = F` でキャッシュを残さない
