...
file.copy(file.path(system.file(package = "rmdzxja"), ".latexmkrc"), to = "./")
# 上記2つは日本語引用文献リストのために必要
knitr::opts_chunk$set(cache = T)
example_dir <- file.path(system.file(package = "rmdzxja"), "examples")
```
...
knitr::include_graphics(file.path(example_dir, "img/render.png"))
```
...
knitr::include_graphics(file.path(example_dir, "img/math-input.png"))
```
* 一部対応してない記号もある?
  + `\mathbb{}`とか`\hat{}`とか
...
knitr::include_graphics(file.path(example_dir, c("img/tiger.eps", "img/tiger.pdf", "img/tiger.png")))
```
...
```
![The Tiger](`r file.path(example_dir, "img/tiger.pdf")`){ height=30% }
...
* markdown方式のリンク
  ...
  + [RStudio](https://rstudio.com/)
* 画像にハイパーリンク [![RStudio](`r file.path(example_dir, "img/RStudio-Logo-flat.pdf")`){ height=10% }](https://rstudio.com/) を貼ることも可
...
knitr::include_graphics(file.path(example_dir, "img/citr.png"))
```
