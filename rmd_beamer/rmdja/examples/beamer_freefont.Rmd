...
date: "`r Sys.Date()`"
output:  rmdja::beamer_presentation_zxja
theme: metropolis
...
knitr::opts_chunk$set(
  ...
  tidy=F,
  # 画像関係: 中央寄せ, 幅50%, 水平並べ
  # 画像サイズはbeamerの画面サイズに合わせている
  fig.align = "center", out.width = "100%",  fig.show='hold'
  # fig.width, fig.height はここで設定可能
  # デバイス=cairo_pdfは日本語フォント埋め込みに必要
  # pointsize は組み込みグラフィックデバイスにのみ反映される
  # dev = "cairo_pdf",
  # dev.args = list(pointsize = base_size)
  )
```
...
* markdown方式のリンク
  ...
  + [RStudio](https://rstudio.com/)
* 画像にハイパーリンク [![RStudio](img/RStudio-Logo-flat.pdf){ height=10% }](https://rstudio.com/) を貼ることも可
...
```{r sushi-plot, echo=T, out.width="40%"}
plot(1:10, pch = "🍣")
```
...
```{yaml header-example1-re, eval=F, echo=T}
...
```{yaml natbib-number, eval=F,echo=T}
