...
knitr::opts_chunk$set(
  ...
  fig.align = "center", out.width = "100%",  fig.show='hold', fig.width = 5.03937, fig.height = 3.77953,
  # デバイス=cairo_pdfは日本語フォント埋め込みに必要
  # pointsize は組み込みグラフィックデバイスにのみ反映される
  dev = "cairo_pdf", dev.args = list(pointsize = base_size)
  )
```
...
## 図の挿入: 画像サイズの基本ルール
* Rが作図したファイルを一旦保存し貼り付けられる
* `fig.width`/`fig.height` は**保存時**のサイズ
* `out.width`/`out.height` は**表示する**サイズ
* 保存した画像がスライドに収まるよう拡大縮小される
  + スライドは**128 mm (5.04 in) x 96 mm (3.78 in)**(4:3)がデフォルト
* `ggsave()`はデフォルトで **9.11 x 5.77** インチで保存
