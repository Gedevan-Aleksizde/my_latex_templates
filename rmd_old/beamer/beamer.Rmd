...
header-includes:
  ...
  - \usepackage{fontspec}
  - \newfontfamily\IPAMin{IPAMincho}
  - \setCJKfamilyfont{IPAMin}{IPAMincho}
---
...
2. 本文中で以下のようにして一部のみ変更
  + フォント名や指定コマンドは自由に変更可
  + 欧文(英数字)と和文(それ以外)で**個別設定が必要**

```md
ここは標準フォント. `{\CJKfamily{IPAMin} ここだけ{\IPAMin IPA}明朝}`{=latex}
```

ここは標準フォント. `{\CJKfamily{IPAMin} ここだけ{\IPAMin IPA}明朝}`{=latex}

* 参考: [詳しい話](http://zrbabbler.sp.land.to/zxjatype.html#ssec-basic-cmd), [`XeCJK`のフォールバックフォントの設定](https://tex.stackexchange.com/questions/45331/combining-chinese-japanese-and-korean-text-with-xecjk)
